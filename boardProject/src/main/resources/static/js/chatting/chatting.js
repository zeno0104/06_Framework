  ...
  fetch("/chatting/roomList")
    ...
    .then((roomList) => {
      ...
      for (let room of roomList) {
        ...
        else listProfile.setAttribute("src", room.targetProfile);
...
function selectChattingFn() {
  fetch(
    "/chatting/selectMessage?" +
      `chattingRoomNo=${selectChattingNo}&memberNo=${loginMemberNo}`
  )
    .then((resp) => resp.json())
    .then((messageList) => {
...
if (loginMemberNo != "") {
  chattingSock = new SockJS("/chattingSock"); // handshake 요청을 보내주는 주소
}
